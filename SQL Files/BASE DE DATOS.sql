...
CREATE TABLE IF NOT EXISTS `tpbd1`.`vehiculo` (
  ...
  `eliminado` BIT NOT NULL,
  `fechaEliminado` DATETIME NULL,
  `detalle_pedido_idPedido` INT(11) NOT NULL,
  `detalle_pedido_idModelo` INT(11) NOT NULL,
  PRIMARY KEY (`idVehiculo`),
  INDEX `fk_vehiculo_detalle_pedido1_idx` (`detalle_pedido_idPedido` ASC, `detalle_pedido_idModelo` ASC),
  CONSTRAINT `fk_vehiculo_detalle_pedido1`
    FOREIGN KEY (`detalle_pedido_idPedido` , `detalle_pedido_idModelo`)
    REFERENCES `tpbd1`.`detalle_pedido` (`idPedido` , `idModelo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;

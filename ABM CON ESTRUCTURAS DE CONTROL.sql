...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `pedido_alta`(id int, cantidad int, concesionaria int, fecha datetime,out resultado int,out mensaje varchar(45))
BEGIN
...
ELSE
insert into pedido(idPedido,idConcesionaria,cantidad,fecha,eliminado) values(id,concesionaria,cantidad,fecha,0);
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `pedido_modificacion`(id int, cantidad int, concesionaria int, fecha datetime,out resultado int,out mensaje varchar(45))
BEGIN
...
select 0 INTO resultado;
select " " into mensaje;
update pedido set idConcesionaria=concesionaria, cantidad=cantidad, fecha=fecha where idPedido=id;
ELSE
select -1 into resultado;
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `detalle_pedido_alta`(idP int, idM int, out resultado int,out mensaje varchar(45))
BEGIN
...
ELSE
insert into detalle_pedido(idPedido,idModelo,eliminado) values(idP,idM,0);
select 0 into resultado;
select " " into mensaje;
END IF;
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `detalle_pedido_modificacion`(idP int, idM int, out resultado int,out mensaje varchar(45))
BEGIN
...
select 0 INTO resultado;
select " " into mensaje;
update detalle_pedido set idModelo=idM where idPedido=idP;
ELSE
select -1 into resultado;
select "No se encontro para modificar" into mensaje;
END IF;
